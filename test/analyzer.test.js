...
const semanticChecks = [
  ...
  ["complex array types", "num f([[[int]]] x) {yeet 3;}"],
  ["increment and decrement", "now x: int = 10; x--; x++;"],
  ["initialize with empty array", "set a: [int] = [];"],
  ...
  ["ok to != arrays", "print([1]!=[5,8]);"],
  ["arithmetic", "set x:int=1;print(2*3+5^-3/2-5%8);"],
  ["array length", "print([1,2,3].length());"],
  ["variables", "now x:[[[[int]]]]=[[[[1]]]]; print(x[0][0][0][0]+2);"],
  ["nested structs", "struct T{y;} struct S{z;} now x: S=S(T(1)); print(x.z.y);"],
